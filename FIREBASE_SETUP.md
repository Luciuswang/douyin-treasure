...
## 📋 步骤3.5：启用 Storage（图片上传）

1. 在Firebase控制台中，点击左侧菜单的"Storage"
2. 点击"开始使用"
3. 选择"以测试模式启动"
4. 选择位置（建议选择 `asia-east1` 或 `asia-southeast1`）
5. 点击"完成"
...
service firebase.storage {
  match /b/{bucket}/o {
    match /treasures/{allPaths=**} {
      // 允许所有用户读取和上传
      allow read: if true;
      allow write: if request.resource.size < 5 * 1024 * 1024; // 限制5MB
    }
  }
}
```
...
对于个人项目和小型应用，免费额度通常足够使用。如果用户量增长，可以考虑：
- 开启付费计划
- 实施更严格的大小限制
- 使用会员制度来控制上传量
